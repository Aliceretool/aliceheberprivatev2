...
WHERE pf.project_fieldID = 'ef2cf9dc-0d9e-4d80-9972-454c7b777ca8' 
AND pf.cascade = 0 
AND pt.project_typeID = {{getIssue4.data.project_typeID[0]}}
AND pv.isActive = 1 AND pt.isActive = 1 AND pf.isActive = 1
AND pv.project_field_type_valueID = {{select_responsibleDepartment2.value}}
...
INNER JOIN project_field_type_value pv ON pt.project_field_project_typeID = pv.project_field_project_typeID
WHERE pf.project_fieldID = 'ef2cf9dc-0d9e-4d80-9972-454c7b777ca8' 
AND pf.cascade = 1 AND pv.isActive = 1 AND pt.isActive = 1 AND pf.isActive = 1 AND pv.project_field_type_valueID = {{select_responsibleDepartment2.value}}