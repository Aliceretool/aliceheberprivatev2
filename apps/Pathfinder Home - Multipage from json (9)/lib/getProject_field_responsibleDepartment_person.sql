...
AND pv.isActive = 1 AND pt.isActive = 1 AND pf.isActive = 1 AND pv.project_field_type_valueID = {{select_responsibleDepartment.value}}
...
WHERE pf.project_field = 'Responsible Department'
AND pf.projectID = {{current_project_id.value}}
AND pf.cascade = 1 AND pv.isActive = 1 AND pt.isActive = 1 AND pf.isActive = 1 AND pv.project_field_type_valueID = {{select_responsibleDepartment.value}}