...
// Key = The exact string from "gh_apprenticeship.risk_prediction_feature_name"
...
// 1. Get the data
const data = {{ individual_learners_query_risk3.data }};
...
const allFactors = data.map(row => {
  const featureName = row['gh_apprenticeship.risk_prediction_feature_name'];
  const originalDesc = row['gh_apprenticeship.risk_prediction_feature_description'];
  ...
  return {
    ...
    // Parse the number
    shapley_val: parseFloat(row['gh_apprenticeship.risk_prediction_feature_shapley_value'] || 0),
    
    updated_at: row['gh_apprenticeship.apprenticeship_latest_risk_prediction_timestamp']
  };
});
