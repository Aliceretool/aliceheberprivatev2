...
const rawData = {{ individual_learners_query_projects3.data }};
...
rawData.forEach(row => {
  const title = row['gh_apprenticeship.project_title'];
  const reviewLink = row['gh_apprenticeship.project_review_link'];
  ...
  if (!uniqueProjectsMap[title]) {
    ...
    const existingHasLink = existingEntry['gh_apprenticeship.project_review_link'] !== null;
...
const uniqueProjects = Object.values(uniqueProjectsMap).map(row => {
  return {
    project_title: row['gh_apprenticeship.project_title'],
    learner_name: row['gh_apprenticeship.learner_full_name'],
    project_link: row['gh_apprenticeship.project_link'],
    review_link: row['gh_apprenticeship.project_review_link'],
    // Boolean helper to easily disable buttons in UI if link is missing
    has_review: !!row['gh_apprenticeship.project_review_link']
  };
});
