const rawData = {{ best_avg_query_current4.data.data }};

const data = (rawData || []).filter(row => 
  Number(row['sidekick__apprenticeship_monthly.apprenticeship_month_active_apprenticeships_count']) > 10
);
...
return {
  ...
  lsat: {
    avg: _.meanBy(data, o => getNum(o, 'sidekick__apprenticeship_monthly.apprenticeship_month_avg_lsat_score_avg')).toFixed(1),
    best: _.max(data.map(o => getNum(o, 'sidekick__apprenticeship_monthly.apprenticeship_month_avg_lsat_score_avg'))).toFixed(1)
  },
  attendance: {
  ...
  reviews: {
    avg: (_.meanBy(data, o => getNum(o, 'sidekick__apprenticeship.progress_review_completed_on_time_pct')) * 100).toFixed(1),
    best: (_.max(data.map(o => getNum(o, 'sidekick__apprenticeship.progress_review_completed_on_time_pct'))) * 100).toFixed(1)
  },
  gateway: {
    avg: (_.meanBy(data, o => getNum(o, 'sidekick__apprenticeship.apprenticeship_gateway_booked_within_day_90_pct')) * 100).toFixed(1),
    best: (_.max(data.map(o => getNum(o, 'sidekick__apprenticeship.apprenticeship_gateway_booked_within_day_90_pct'))) * 100).toFixed(1)
  },
  active: {
    avg: _.meanBy(data, o => getNum(o, 'sidekick__apprenticeship_monthly.apprenticeship_month_active_apprenticeships_count')).toFixed(0),
    max: _.max(data.map(o => getNum(o, 'sidekick__apprenticeship_monthly.apprenticeship_month_active_apprenticeships_count')))
  },
  bil: {
