...
>
  ...
  <Body>
    <ListViewBeta
      ...
      padding="0"
    >
      <Include src="./viewNotesContainer.rsx" />
    </ListViewBeta>
  </Body>
  <Footer>
    <RadioGroup
      id="noteTypeRadioGroup"
      groupLayout="wrap"
    ...
    >
      ...
      <Event
        id="516a433e"
        enabled={'{{ noteTypeRadioGroup.value !== "intervention" }}'}
        event="change"
        method="clearValue"
        params={{}}
        pluginId="interventionActionsMultiselect"
        type="widget"
        waitMs="0"
        waitType="debounce"
      />
      <Event
        id="115cf663"
        enabled={'{{ noteTypeRadioGroup.value !== "support_factor" }}'}
        event="change"
        method="clearValue"
        params={{}}
        pluginId="supportFactorMultiselect"
        type="widget"
        waitMs="0"
        waitType="debounce"
      />
    ...
    <Multiselect
      id="supportFactorMultiselect"
      disabled="{{ postLearnerNote.isFetching || postInterventionNote.isFetching || postSupportFactorNote.isFetching }}"
      emptyMessage="No options"
      hidden="{{ noteTypeRadioGroup.value !== 'support_factor' }}"
      itemMode="static"
      label="Support Factors"
      labelPosition="top"
      loading="{{ postLearnerNote.isFetching || postInterventionNote.isFetching || postSupportFactorNote.isFetching }}"
      overlayMaxHeight={375}
    ...
    <Select
      id="interventionActionsMultiselect"
      disabled="{{ postLearnerNote.isFetching || postInterventionNote.isFetching || postSupportFactorNote.isFetching }}"
      emptyMessage="No options"
      hidden="{{ noteTypeRadioGroup.value !== 'intervention' }}"
      itemMode="static"
    ...
    >
      ...
      <Event
        ...
        method="trigger"
        params={{}}
        pluginId="getInterventionTypeOptions"
        type="datasource"
        waitMs="0"
        waitType="debounce"
      />
    ...
    <Select
      id="interventionTypeMultiselect"
      data="{{ getInterventionTypeOptions.data }}"
      disabled="{{ 
  postLearnerNote.isFetching ||
  postInterventionNote.isFetching ||
  postSupportFactorNote.isFetching ||
  !interventionActionsMultiselect.value
}}"
      emptyMessage="No options"
      hidden="{{ noteTypeRadioGroup.value !== 'intervention' }}"
    ...
    >
      <Event
        ...
        params={{
          map: {
            src: 'const raw = interventionTypeMultiselect.value;\n\nconst selectedValue = Array.isArray(raw)\n  ? raw[raw.length - 1]\n  : raw;\n\nconst normalizedValue =\n  selectedValue && typeof selectedValue === "object"\n    ? selectedValue.value\n    : selectedValue;\n\nconst match = (getInterventionTypeOptions.data || []).find(\n  item => item.value === normalizedValue\n);\n\nnotesInputTextBox.setValue(interventionTypeMultiselect.selectedItem?.templateText || "");',
          },
        }}
        pluginId=""
        type="script"
...
"
    ...
    >
      <Event
        id="6009cfc5"
        enabled="{{ noteTypeRadioGroup.value == 'note'}}"
        event="click"
        method="trigger"
        params={{}}
        pluginId="postLearnerNote"
        type="datasource"
        waitMs="0"
        waitType="debounce"
      />
      ...
      <Event
        ...
        method="trigger"
        params={{}}
        pluginId="postSupportFactorNote"
        type="datasource"
        waitMs="0"
        waitType="debounce"
      />
