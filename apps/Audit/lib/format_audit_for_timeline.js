...
const userDict = {{formatusers.value}}
  ...
  //Sets up all differences with ID-columns
   for (const key in oldRecord) {
    ...
    output[i].differencesNoID = differencesNoID;
        delete output[i].old_record;
      delete output[i].record;
//  Below is the 
...
// Some code snippets to create more easily accessible objects to populate audittrail
    ...
    output[i].timelineOutput = output[i].op + ' by: ' + output[i].created_by +' '+ output[i].differencesNoID.map(x => x.field + ' <br/> ' +' field from ' + '<del> ' + x.old_value + '</del>' + ' to ' +  '<ins>' + x.new_value + '</ins>'+ ' <br>')
        ...
        // Assuming output is the array and ts is a timestamp in ISO 8601 format
    for (let i = 1; i < output.length; i++) {
      const currentTs = moment(output[i-1].ts);
...
output.forEach(item => {
  if(item.differencesNoID.length == 0){item.differencesNoID.push({
              'field': 'No edits made',
              'old_value': '',
              'new_value': ''
            })}
...
item.differencesNoID.forEach(difference => {
  ...
  if (fieldNameMapping[difference.field]) {
    ...
    if (userDict[difference.old_value]) {
    difference.old_value = userDict[difference.old_value];
  }
  if (userDict[difference.new_value]) {
    difference.new_value = userDict[difference.new_value];
  }
...
//output = output.filter(item => item.differencesNoID.length > 0 && item.op != 'INSERT');
//newest first
